<h1 align="center">Hi 👋, I'm Majeed Mujawar</h1>
<h3 align="center">Aspiring DevOps & Cloud Engineer from India</h3>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&center=true&vCenter=true&width=435&lines=AWS+%7C+Linux+%7C+Networking;DevOps+Enthusiast;Learning+Cloud+Engineering;Always+Learning+New+Things" alt="Typing SVG" />
</p>

---

## 🚀 About Me

- 🌱 Currently learning **DevOps, AWS Cloud & Linux**
- ☁️ Exploring **CI/CD, Docker, Kubernetes & Terraform**
- 💻 Passionate about **Cloud Infrastructure & Automation**
- 🛠️ Building hands-on projects to become job-ready
- 📚 Learning through labs, projects & real-world practice
- 🎯 Goal: Become a skilled **Cloud/DevOps Engineer**

---

## 🧰 Tech Stack & Tools

<p align="left">

<img src="https://skillicons.dev/icons?i=aws,linux,docker,kubernetes,terraform,git,github,bash,python,mysql,vscode" />

</p>

---

## ☁️ Currently Learning

```yaml
Cloud:
  - AWS EC2
  - IAM
  - VPC
  - S3
  - CloudWatch

DevOps:
  - Docker
  - Kubernetes
  - CI/CD
  - GitHub Actions

System:
  - Linux Administration
  - Networking Basics
  - Shell Scripting
```

---

## 🌐 Connect With Me

<p align="left">
<a href="https://linkedin.com/in/majeedmujawar" target="blank">
<img align="center" src="https://skillicons.dev/icons?i=linkedin" height="40" />
</a>

</p>

---

## ⚡ Fun Fact

```bash
I use Linux because clicking less feels more productive 😄
```

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=majeed69&label=Profile%20Views&color=0e75b6&style=flat" alt="profile views" />
</p>
